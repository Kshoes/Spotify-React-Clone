...
const Sidebar = () => {
  const [sidebarState, setState] = useState({       // sidebarState is local state in Sidebar.js
    modal: false,
    toast: ''
  })
  ...
  const addPlaylist = e => {
    ...
    dispatch({ type: 'ADD_PLAYLIST', playlist: list })  // update global state/context
  ...
  return (
    <ul className="Sidebar" css={CSS}>
        ...
        <Modal show = {sidebarState.modal} close = {handleModal}       // show state makes modal visible, close state sets it back to invisible
            >
        ...
        <Toast toast={sidebarState.toast}
            close={() => {
