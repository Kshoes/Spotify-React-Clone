...
const Toast = ({ toast, close }) => {
  useEffect(() => {
    if (!toast) return          // if toast state exists, run code below, otherwise don't

    const closeToast = () => {      // execute 'close' callback function after 2.5 seconds, which closes the toast
      setTimeout(() => {
        close()
      }, 2500)
    }
